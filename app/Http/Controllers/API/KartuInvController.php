...
class KartuInvController extends Controller
{
    public function store_keris(Request $request)
    {
        ...
        $request->validate([
            'foto_inv' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048' // Menyatakan bahwa `foto` harus berupa file gambar dengan format yang diizinkan dan ukuran maksimal 2MB
        ]);

        $kartu_inv = new kartu_inv;
        $kartu_inv->id_koleksi = $request->input('id_koleksi');
        $kartu_inv->id_buku = $request->input('id_buku');
        ...
        $kartu_inv->ur_bentuk = $request->input('ur_bentuk');
        ...
        $kartu_inv->sumber_literatur = $request->input('sumber_literatur');
    ...
    public function update(Request $request,$id_kartuinventaris)
    {
        ...
        $kartu_inv = kartu_inv::findOrFail($id_kartuinventaris);
        $kartu_inv->id_koleksi = $request->input('id_koleksi');
        $kartu_inv->id_buku = $request->input('id_buku');
        $kartu_inv->jenis_koleksi = $request->input('jenis_koleksi');
        $kartu_inv->sub_koleksi = $request->input('sub_koleksi');
